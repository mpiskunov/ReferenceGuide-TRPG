...
export class SelfPopulatingTableComponent implements OnInit {
  ...
  performFilter(filterBy: string): ITableDetail[] {
    ...
    this.filteredItemArray = this.filteredItemArray.filter(
      (item: ITableDetail) =>
        item.columns[this.indexOfName].itemValue
          .toLocaleLowerCase()
          .indexOf(filterBy) !== -1
    );
  ...
  refreshViewableItems = (newActivePage: number) => {
    ...
    this.filteredItemArray = this.filteredItemArray.slice(
      startingIndex,
      endingIndex
    );
  };
  ...
  refreshItemArray() {
    ...
    if (this.sortAlphabetically) {
      ...
      this.filteredItemArray.sort(
        (a, b) =>
          +a.columns[this.indexOfID].itemValue -
          +b.columns[this.indexOfID].itemValue
      );
    }
